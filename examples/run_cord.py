...
def main():
    ...
    num_labels = int((len(label_list)-1)/2)
    ...
    from model.hgalayoutlm import HGALayoutLMForTokenClassification
    # from model.modeling_graphlayoutlm_gp import LayoutLMv3GlobalPointerForTokenClassification
    model=HGALayoutLMForTokenClassification.from_pretrained(
    ...
    def tokenize_and_align_labels(examples, augmentation=False):
        ...
        for nodes_data,edges_data in zip(nodes,edges):
            ...
            graph_mask= torch.zeros((input_len,input_len), dtype=torch.bool)#-9e15 * np.ones((input_len,input_len))
            for edge_i in range(edges_len):
                ...
                graph_mask[a_start:a_end+1,b_start:b_end+1]=True
