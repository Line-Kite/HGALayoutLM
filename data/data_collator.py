...
class DataCollatorForKeyValueExtraction(DataCollatorMixin):
    ...
    def __call__(self, features):
        ...
        if labels is not None:
            other_label = 2 * self.num_labels
            zeros = torch.zeros((labels.shape[0], self.num_labels, labels.shape[1], labels.shape[1]), dtype=torch.bool, device=labels.device)
            for b, labels_i in enumerate(labels):
                label_type = -1
                for i, label in enumerate(labels_i):
                    if attention_mask[b, i] == 0 or i == input_ids.shape[1]:
                        break
                    label = int(label.item())
                    if label != -100:
                        if label_type == -1 and label != other_label:
                            label_type = label
                            l_start = i
                        elif label % 2 == 0 and label != other_label:
                            zeros[b, int(label_type / 2), l_start, i - 1] = 1
                            label_type = label
                            l_start = i
                ...
                if label_type != -1:
                    zeros[b, int(label_type / 2), l_start, i - 1] = 1
        batch["matrix_labels"] = zeros
        
        batch["graph_mask"]=torch.tensor([feature["graph_mask"] for feature in features], dtype=torch.bool)
